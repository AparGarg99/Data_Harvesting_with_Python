...
def app():
    ...
    if(error_flag==False):
        directory = './instagram2_checkpoints'
        ...
        for i,url in enumerate(urls):
            ...
            try: 
                ...
                time.sleep(3*60) # 3 min wait for manual scroll

                x1 = url
                
                # Get following usernames
                try:
                    element = driver.find_elements_by_css_selector('span[class="_aacl _aaco _aacw _adda _aacx _aad7 _aade"]')
                    x2 = [i.text for i in element if i.text!='']

                    if(x2==[]):
                        element = driver.find_elements_by_css_selector('div[class="_ab8y  _ab94 _ab97 _ab9f _ab9k _ab9p _abcm"]')
                        x2 = [i.text for i in element if i.text!='']    
                ...
                record.columns = ['Account_url','Following_usernames']
                #print(record.shape)
                print(record)
            ...
            pd.concat(df5).to_csv(f'./instagram2_checkpoints/checkpoint{i}.csv',index=False)
    ...
    if(button==True and error_flag==False):
        ...
        st.write(df5)
        df5 = df5.explode('Following_usernames').reset_index(drop=True)
