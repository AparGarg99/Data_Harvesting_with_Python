...
class biospace_news:
    ...
    # Scrape links and headlines from first 5 pages of BioSpace
    def parse_pages(self, driver, query):
        ...
        for i in range(1,npages+1):
            # open page
            if(query==''):
                driver.get(f'https://www.biospace.com/news/{i}/')
            else:
                driver.get(f'https://www.biospace.com/news/{i}/?Keywords={query}')
    ...
    def scrape(self, query, start, end):
        ...
        save_data(filename='e_biospace_news.xlsx', path=paths['OUTPUT_DIR'], df=df)
